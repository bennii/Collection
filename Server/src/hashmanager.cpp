...
int Hashmanager::contains(HASH * _hash)
{
	...
	{
		if (iter->gameHash == _hash->gameHash && iter->completeHash == _hash->completeHash) return iter->hashNumber;
	}
...
int Hashmanager::contains(std::string _hash)
{
	...
	for(iter = hashTable.begin(); iter != hashTable.end(); iter++)
	{
		if (iter->gameHash == _hash || iter->completeHash == _hash) return iter->hashNumber;
	}
