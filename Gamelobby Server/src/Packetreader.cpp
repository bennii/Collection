...
{
	if (packet == NULL || index + sizeof(unsigned char) > packet->size) return 0;
...
{
	if (packet == NULL || index + sizeof(unsigned short) > packet->size) return 0;
...
{
	if (packet == NULL || index + sizeof(unsigned int) > packet->size) return 0;
...
{
	if (packet == NULL || index + sizeof(double) > packet->size) return 0.0;
